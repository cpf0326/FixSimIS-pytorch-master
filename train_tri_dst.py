...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        pseudo_label_ratios = AverageMeter()
        ...
        for batch_idx in range(args.eval_step):
            ...
            if not args.no_progress:
                p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {loss:.4f}. wce_losses: {wce_losses:.4f}. cls_loss: {cls_loss:.4f}. self_training_loss: {self_training_loss:.4f}.  Wce_losses: {wce_losses:.4f}. Mask: {mask:.2f}. ".format(
                    epoch=epoch + 1,
                    epochs=args.epochs,
                    batch=batch_idx + 1,
                    iter=args.eval_step,
                    ...
                    bt=batch_time.avg,
                    loss=losses.avg,
                    cls_loss=cls_loss.avg,
                    self_training_loss=self_training_loss.avg,
        ...
        if args.local_rank in [-1, 0]:
            ...
            args.writer.add_scalar('train/2.train_loss_x', cls_loss.avg, epoch)
            args.writer.add_scalar('train/3.train_loss_u', self_training_loss.avg, epoch)
